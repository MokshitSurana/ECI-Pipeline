...
periodically fetches a new snapshot, computes a structured diff against
the previous one, and produces a change event $\Delta_i(t)$ capturing the
added text, deleted text, change type, and source metadata. Unlike naive
textual diffing, it applies HTML cleaning, boilerplate removal, and
cosmetic-change filtering before comparison.

\textbf{Stage 2: Change-Grounded Retrieval.} Only the delta content is
chunked, embedded with a task-aware model, and stored in a vector index;
restricting embedding to deltas rather than full documents concentrates
the index on operationally relevant signal and avoids the vector dilution
that plagues full-document RAG over long, repetitive corpora. In parallel,
a knowledge graph (KG) layer indexes entities extracted from each change
(CVEs, APIs, permissions, policy clauses) and links them to their
originating change. At query time the system fuses vector retrieval over
the delta index with a 2-hop KG traversal via Reciprocal Rank Fusion; we
call this fused retrieval \textbf{DeltaRAG}.

\textbf{Stage 3: Evidence-Backed Action Generation.} For changes deemed
relevant by the Sentinel Agent, a Coordinator Agent consumes the DeltaRAG
output, identifies cross-source connections (e.g., a CVE linked to an API
deprecation linked to a policy change), and generates Action Tickets with
structured fields (summary, risk score, recommended actions, priority,
owner, evidence citations), constrained to a strict JSON schema for
downstream parseability.

Figure~\ref{fig:workflow} and Algorithm~\ref{alg:eci}
(Section~\ref{system-architecture}) give the end-to-end workflow, and
Section~\ref{formal-framework} formalizes each stage.
...
We now formalize the core ECI operations. Recall the monitored sources
$\mathcal{S}$ from Section~\ref{the-operational-gap}; each $s_i$ produces a
time-ordered snapshot sequence:
\begin{equation}
...
\noindent These three definitions are exactly the variants compared in our ablation (Section~\ref{ablation-study}): \textbf{V1 Standard RAG} is the vector retrieval $R_{\mathrm{vec}}$ (Eq.~5), \textbf{V2 KG-Only} is the graph context expansion $\mathrm{KGCtx}$ (Eq.~8), and \textbf{V3 DeltaRAG} is their RRF fusion (Eq.~10). The evaluation therefore isolates the contribution of each component of the framework rather than treating DeltaRAG as a monolithic system.
...
The ECI system is organized as a six-stage pipeline spanning three architectural layers: the Platform Evolution Layer (data acquisition and change detection), the Semantic Retrieval Layer (embedding, storage, and similarity search), and the Agent Intelligence Layer (triage, coordination, and recommendation generation). Figure~\ref{fig:workflow} shows the end-to-end workflow and Algorithm~\ref{alg:eci} gives the corresponding procedure; we describe each component below.
...
\begin{tikzpicture}[
  ...
  base/.style={draw, rounded corners=2pt, align=center, inner sep=3pt,
               minimum height=7mm, text width=30mm},
  plat/.style={base, fill=blue!8},
  sem/.style={base, fill=teal!12},
  agent/.style={base, fill=orange!14},
  flow/.style={->, semithick, draw=black!65},
  qry/.style={->, semithick, dashed, draw=black!55},
]
% main execution spine
\node[plat]               (src)  {Monitored sources \scriptsize(registry)};
\node[plat, below=of src] (snap) {Snapshot ingestion\\\scriptsize fetch $\cdot$ clean $\cdot$ hash};
\node[plat, below=of snap](diff) {Change detection\\\scriptsize structured diff $\Delta$};
\node[agent, below=of diff](sent){Sentinel triage\\\scriptsize relevance $\geq \tau$?};
\node[sem, below=of sent]  (rrf) {DeltaRAG retrieval\\\scriptsize $R_{\mathrm{vec}} \oplus$ 2-hop KG $\to$ RRF};
\node[agent, below=of rrf] (coord){Coordinator synthesis};
\node[agent, below=of coord](tick){Action Ticket\\\scriptsize summary $\cdot$ risk $\cdot$ actions $\cdot$ evidence};
% indexing side branch
\node[sem, right=of diff]  (emb) {Delta embedding\\\scriptsize $\to$ vector index $\mathcal{D}$};
\node[sem, below=of emb]   (kg)  {Entity extraction\\\scriptsize $\to$ knowledge graph $\mathcal{G}$};
% flows
\draw[flow] (src)  -- (snap);
\draw[flow] (snap) -- (diff);
\draw[flow] (diff) -- (sent);
\draw[flow] (sent) -- node[right=1pt, font=\scriptsize]{if relevant} (rrf);
\draw[flow] (rrf)  -- (coord);
\draw[flow] (coord)-- (tick);
\draw[flow] (diff) -- (emb);
\draw[flow] (emb)  -- (kg);
\draw[qry]  (emb.south east) to[out=-30,in=30] (rrf.east);
\draw[qry]  (kg)  -- (rrf.east);
\end{tikzpicture}
\caption{ECI end-to-end workflow. Node colour denotes architectural layer:
\textcolor{blue!55!black}{blue} = Platform Evolution, \textcolor{teal}{teal}
= Semantic Retrieval (DeltaRAG), \textcolor{orange!80!black}{orange} = Agent
Intelligence. Solid arrows are the execution spine; dashed arrows show the
continuously built vector index $\mathcal{D}$ and knowledge graph $\mathcal{G}$
being queried at retrieval time.}
\label{fig:workflow}
\end{figure}

\begin{algorithm}[t]
\caption{ECI: from ecosystem change to Action Ticket}
\label{alg:eci}
\begin{algorithmic}[1]
\Require sources $\mathcal{S}$, relevance threshold $\tau$, depth $k$, fusion constant $k_{\mathrm{rrf}}$
\Statex \textit{// Continuous, delta-only indexing}
\ForAll{$s_i \in \mathcal{S}$}
  \State $\Delta_i \gets \textsc{Diff}\big(\textsc{Fetch}(s_i),\, s_i(t_{k-1})\big)$
  \If{$\Delta_i$ exceeds the cosmetic-change threshold}
    \State $\mathcal{D} \gets \mathcal{D} \cup \textsc{Embed}(\textsc{Chunk}(\Delta_i))$ \Comment{delta-only vector index}
    \State $\mathcal{G} \gets \textsc{UpdateKG}\big(\mathcal{G},\, \textsc{Entities}(\Delta_i)\big)$
  \EndIf
\EndFor
\Statex \textit{// Per pending change: triage, then generate}
\ForAll{pending change $\Delta$ with text $q$}
  \State $\mathit{tri} \gets \textsc{Sentinel}(\Delta)$ \Comment{LLM first-pass triage}
  \If{$\mathit{tri}.\mathrm{relevance} \geq \tau$}
    \State $R_{\mathrm{vec}} \gets \operatorname{top\text{-}}k_{\,c \in \mathcal{D}}\, \cos\!\big(\phi_q(q),\, \mathbf{e}_c\big)$
    \State $K \gets \{\,c : c \text{ linked to } N_2(\mathcal{E}(q)) \text{ in } \mathcal{G}\,\}$
    \State $F \gets \textsc{RRF}(R_{\mathrm{vec}},\, K;\, k_{\mathrm{rrf}})$ \Comment{DeltaRAG fusion}
    \State $A \gets \textsc{Coordinator}(\Delta,\, F)$ \Comment{summary, risk, actions, evidence}
    \State \textbf{emit} Action Ticket $A$
  \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
...
\subsubsection{Snapshot Ingestion}
...
Change content is chunked following Eq.~3 (the 1600/200 rule), balancing granularity (each chunk stays semantically focused) against context preservation (avoiding mid-sentence breaks). Before embedding, each chunk is prefixed with \texttt{search\_document:} to activate Nomic v1.5's task-aware encoding path (Eq.~4), optimizing the embedding strictly for retrieval rather than classification.

\subsubsection{Embedding Model: Nomic v1.5}

We selected Nomic Embed v1.5 (the selection criteria are detailed in Section~\ref{retrieval-quality}). For the architecture, its decisive properties are: 768-dimensional embeddings that capture nuanced distinctions between vulnerability classes; an 8,192-token input length that accommodates long JSON feeds and policy documents without truncation; and local execution, ensuring zero-day vulnerability data never leaves controlled infrastructure.

\subsubsection{Vector Storage: Supabase + pgvector}
...
\subsubsection{Sentinel Triage Agent}
...
\subsubsection{Coordinator Agent (KG-Augmented Retrieval)}
...
\subsubsection{Pipeline Orchestration \& Logging}
...
At query time the system runs vector search (Eq.~5) and 2-hop KG traversal
(Eqs.~7--8) in parallel and merges them with Reciprocal Rank Fusion
(Eq.~9, $k_{\mathrm{rrf}} = 60$), yielding the DeltaRAG retrieval of
Eq.~10. The design choice worth emphasizing is the use of RRF over naive
list concatenation or graph-distance boosting: because RRF combines
\emph{ranks} rather than raw scores, a chunk ranked highly by vector
search retains competitive standing even when the KG contributes nothing.
Extraction blackouts therefore degrade gracefully to the vector baseline
rather than catastrophically; the residual cost, when both signals are
active but disagree, is the fusion regression characterized in
Section~\ref{failure-analysis}.
...
RRF fusion is visible on thirteen of the 85 scored queries, where V3
recall falls below the vector-only V1 (e.g.\ A09: 0.80$\to$0.60; B01,
E01, E07, E09, J01: 1.00$\to$0.75; A01, A03, A04: 1.00$\to$0.80; D03,
F03: 1.00$\to$0.67). The
...
To move beyond a prose account, we classify every one of the 85 scored
queries into a single dominant failure category by a mechanical rule over
the extractor and the three retrievers (a query is assigned to fusion
demotion if V3 recall drops below V1; otherwise to a KG-blackout category
if 2-hop traversal returns nothing relevant, split by whether any entity
was extracted; otherwise to incomplete-linkage if the graph fires but
recovers only part of the gold set). Table~3 reports the resulting
distribution. The decomposition is itself diagnostic: of the 54 queries
that exhibit any imperfection, only the 13 fusion demotions (C4) are a
\emph{net} harm; the other 41 (C2, C3, C5) degrade gracefully because RRF
falls back to, or is completed by, the vector ranking, so DeltaRAG recall
matches or exceeds V1 on every one of them. The single largest bucket,
C2, also sharpens the F1 narrative: the regex extractor almost never fails
to capture a well-formed identifier (C1 = 0); rather, the extracted entity
is a singleton (a CVE or SVE present in only one source) or a component
name with no cross-source co-occurrence edge, so the graph has nothing to
traverse to.

{\def\LTcaptype{none}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.20}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.05}}
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.23}}
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.26}}
  >{\raggedright\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.26}}@{}}
\toprule\noalign{}
\textbf{Failure category} & \textbf{n} & \textbf{Representative} & \textbf{Effect on retrieval/output} & \textbf{Interpretation} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
C1: Entity not captured by regex & 0 & --- (none observed) & No occurrences. & Well-formed CVE/SVE/CVSS identifiers are reliably matched; pure extraction misses do not arise. The extractor's limitation surfaces instead as C2. \\
C2: Implicit relation not captured & 19 & C09 ``Knox attestation bypass\ldots''; B07 (CVE-2025-0098, Camera HAL) & KG-only returns $\emptyset$ (Graph R@5 = 0.00); V3 falls back to vector and preserves recall (e.g.\ 1.00). & An entity \emph{is} extracted, but it has no co-occurrence edge: a singleton identifier in one source, or a component (Knox, Titan, Camera HAL) with no cross-source link. Graceful. \\
C3: NL query, no extractable identifier & 11 & G01 ``new kernel vulns added between snapshots''; H04 ``by what date must agencies patch\ldots'' & KG inert (Graph R@5 = 0.00); retrieval reduces to vector-only; recall intact where embeddings suffice, $\approx$0.50 on under-specified phrasings. & Temporal/compliance/analyst phrasings carry no rigid identifier to seed traversal --- the clearest target for LLM-based extraction. \\
C4: Fusion demotes a relevant item & 13 & A09 (0.80$\to$0.60); B01, E09 (1.00$\to$0.75) & A true-positive source is pushed past rank 5; mean recall regression; main driver of V3's depth-5 deficit vs.\ V1. & Both signals confident but disagreeing; the cross-category bonus lifts a connected non-gold chunk over a relevant vector hit. \emph{Net harm.} \\
C5: Incomplete structural linkage & 11 & D01 (Graph R@5 = 0.50, V3 = 1.00); F01 & KG recovers only part of the gold set; fusion with the vector ranking usually completes it (V3 R@5 = 1.00). & 2-hop traversal reaches some but not all linked sources (partial co-occurrence, hub-entity stoplisting). Partial signal, no net harm. \\
\end{longtable}
}

\emph{Table 3: Structured error analysis over the 85 scored queries
(31 exhibit no failure). Categories are mutually exclusive and assigned by
mechanical rule at $k=5$; C4 is the only net-harmful mode, and refines
F1--F2 with per-category counts, mechanisms, and effects.}
...
specific to that depth (Table~4). At $k = 3$ the recall gap is not
...
\emph{Table 4: DeltaRAG sensitivity to retrieval depth $k$. Metrics are
...
\subsection{Latency and Operational Overhead}
\label{latency-overhead}

To assess whether the KG augmentation is operationally affordable, we
profile end-to-end Action-Ticket generation across the ten escalated
changes, decomposing it into its main components (Table~5). The
deterministic retrieval stages are averaged over three runs each; the
two LLM stages (Sentinel triage and Coordinator synthesis, Groq-hosted
Llama~3.1~8B) are timed once per change.

{\def\LTcaptype{none}
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.46}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.27}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.27}}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Vector retrieval & 669 $\pm$ 132 & 2.4\% \\
KG extraction + 2-hop traversal & 890 $\pm$ 453 & 3.2\% \\
Fusion / reranking (RRF) & $<$1 & $<$0.1\% \\
LLM synthesis (Sentinel + Coordinator) & 26{,}490 & 94.4\% \\
\textbf{Total (end-to-end)} & \textbf{28{,}048} & 100\% \\
\end{longtable}
}

\emph{Table 5: Per-component latency of Action-Ticket generation
(mean across 10 changes). LLM stages are hosted-API wall-clock and
dominate; the KG/fusion machinery is sub-second.}

Two points follow. First, the structural machinery that distinguishes
DeltaRAG from a vanilla pipeline --- entity extraction, 2-hop traversal,
and RRF --- costs about 0.89~s per ticket (the RRF fusion itself is
sub-millisecond, pure in-memory rank arithmetic). Measured against a
vector-only baseline that omits the KG path and the cross-source evidence
block, the \emph{added overhead} of the full DeltaRAG pipeline is
$\approx$0.97~s per ticket ($\approx$3.5\% of end-to-end time): 0.89~s of
KG/fusion plus $\approx$0.08~s for the larger Coordinator prompt. Second,
the dominant cost ($>$94\%) is LLM synthesis, which both the baseline and
DeltaRAG incur identically. We report the LLM figure as hosted-inference
wall-clock on a shared free-tier endpoint; its absolute value reflects
network and provider queueing and will vary with deployment, whereas the
architecturally relevant quantity --- the retrieval-and-fusion overhead
DeltaRAG adds --- is sub-second and effectively negligible next to the
generation step a monitoring system must pay regardless. The
performance-vs-complexity trade is thus favourable: the KG augmentation's
top-rank gains (Section~\ref{ablation-study}) come at a sub-second,
single-digit-percent latency cost.
